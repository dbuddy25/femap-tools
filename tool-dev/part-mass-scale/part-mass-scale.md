# Part Mass Scale Tool

Femap API custom tool (.BAS script) that scales the mass of user-selected elements by modifying material densities (including PCOMP ply materials), NSM, and CONM2 mass values. Includes a built-in verification step that recalculates mass after scaling and compares against the expected value.

## Workflow

1. User selects elements
2. Tool calculates original mass via `feMeasureMeshMassProp`
3. User enters a scale factor in a dialog (original mass displayed)
4. Tool classifies properties from the selection (CONM2 vs structural vs PCOMP)
5. Tool checks for shared materials/properties and warns if any are used by elements outside the selection
6. Tool scales material densities, NSM on structural properties, and CONM2 mass
7. Tool recalculates mass and reports PASS/MARGINAL/FAIL based on % difference

## What Gets Scaled

| Category | API Field | Description |
|---|---|---|
| Material density | `mval(49)` (DENSITY) | Structural mass from volume elements, shells, beams, etc. |
| PCOMP ply material density | `mval(49)` via Layup object | Per-ply materials from `ly.matlID(ply)` on Laminate properties (type 21/22) |
| NSM (non-structural mass) | `pval(7)` (most types), `pval(1)` (laminate), `pval(27)` (beam end B) | Per-property NSM on line/shell elements |
| CONM2 mass | `pval(7)`, `pval(11)`, `pval(12)` | Mx, My, Mz on mass element properties |
| CONM2 MMOI | `pval(1)`–`pval(6)` | I11, I21, I22, I31, I32, I33 (API PDF 5-1036) |

### NSM `pval` Index by Property Type
...
## Property Classification (Section 4)

Instead of inspecting each element's type, the tool collects unique property IDs from the selection, then classifies each property by its `pr.type` field:

- **`pr.type = FET_L_MASS`**: CONM2 mass element. Property ID added to `massPropSet` for mass scaling.
- **`pr.type = 21 or 22` (Laminate/PCOMP)**: `pr.matlID` is **not used** for laminates (API 5-1017 documents it as 0). Instead, ply materials are read from the Layup object via `pr.layupID`. Each ply's `ly.matlID(ply)` is added to `matlSet` for density scaling. The property is also tracked in `pcompPropSet` for sharing detection.
- **All other structural types**: `pr.matlID` added to `matlSet` for density scaling. If the property type supports NSM and the NSM value is nonzero, the property ID is also added to `nsmPropSet` for NSM scaling.

This is simpler and more robust than per-element type checking — the property's own type field is the authoritative source.

## Shared Resource Detection

Before scaling, the tool checks whether any material, CONM2 property, NSM property, or PCOMP property in the selection is also used by elements *outside* the selection. For each material/property:

1. `AddRule` finds all elements using that material/property
2. `RemoveSet` subtracts the user's selection
3. If any remain, the resource is shared

PCOMP properties get an additional check via `FGD_ELEM_BYPROP` since their ply materials live on a shared Layup object — scaling the material affects all elements referencing that layup.

If shared resources are found, a Yes/No message box lists each shared material/property with counts and warns that scaling will affect all elements using them.

## Verification

After scaling, `feMeasureMeshMassProp` is called again on the same element set. Verification is split into **structural** and **non-structural** categories for better diagnostics — if one side is off, the user immediately sees which category has the discrepancy.

**Per-category checks** — For each category (structural, non-structural) with nonzero original mass, the tool computes:
- **% difference**: `(actual - original * scaleFactor) / (original * scaleFactor) * 100`
- **CG shift**: Euclidean distance between before/after CG for that category

**Total check** — % difference between actual total mass and expected total mass (`totalMass0 * scaleFactor`), plus total CG shift.

**PASS/FAIL** uses the **worst** of all per-category and total diffs/CG shifts:

| Condition | Result | Message Color |
|---|---|---|
| worst diff < 0.01% and max CG shift < 1e-6 | PASS | Highlight (magenta) |
| worst diff < 1% | MARGINAL | Warning (orange) |
| worst diff >= 1% | FAIL | Error (red) |

A CG shift >= 1e-6 triggers a separate warning (may indicate non-uniform scaling).

Category lines are only shown when that category has nonzero original mass. Connection Region NSM (not auto-scaled) will cause a non-structural mass discrepancy when present — the structural diff will be clean while the non-structural diff shows the unscaled CR NSM contribution.

## Known Limitations

- **Connection Region NSM**: Connection Regions can carry non-structural mass via `MassNSM` (API 5-387). The tool **detects** Connection Regions with nonzero NSM and reports the count in the summary, but does **not** auto-scale them — Connection Regions are model-level entities not directly tied to specific elements in the selection, so automatic scaling could affect unrelated parts of the model. Uses `femap.ConnectionRegion` / `App.feConnectionRegion` (unverified — needs testing in Femap).

## Script (Section 6 notes)

Section 6 has three scaling steps plus a detection step:
- **6a**: Material densities (`mval(49)`) — includes PCOMP ply materials collected via Layup
- **6b**: NSM on structural properties (`pval(7)`, `pval(27)`, `pval(1)` depending on type)
- **6c**: CONM2 mass/MMOI (`pval(1)`–`pval(7)`, `pval(11)`, `pval(12)`)
- **6d**: Connection Region NSM detection (report only — iterates all Connection Regions, counts those with nonzero `MassNSM`)
