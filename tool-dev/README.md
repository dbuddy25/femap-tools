...
Sets each RBE2's thermal expansion coefficient from the material it is actually attached to, for a picked set of elements or the whole model in one pass. Walks each spider's dependent nodes to the non-rigid elements on them, resolves those elements' materials to `mval(36)`, and writes `el.RigidThermalExpansion`. Rigids are stripped from the attached-element list first, so a spider tied to another spider cannot read a CTE through it. Conflicts are judged on the CTE **value** within a user-set relative tolerance rather than on material ID — two aluminium materials sharing a CTE are one answer, not an ambiguity — and any spider that genuinely spans two different CTEs is skipped with its existing value untouched, printed with the competing values, and collected into a group. Spiders with no material to read (attached only to rigids, masses or plot-only elements) are reported separately from conflicts. RBE3s are counted and skipped, since their node roles are reversed. RBE2s that already carry a CTE are left alone by default, and any whose existing value DISAGREES with the derived one is listed individually - that is either a deliberate hand edit or a stale value left behind when the material under the spider changed, and the second is a wrong model rather than a preference. Report-only mode changes nothing.

**Use case:** Get CTE onto every bolt-hole spider in a model so thermal cases behave, without checking each one by hand — and see immediately which spiders straddle a material joint and need a decision.

### export-nsm-cards
Extracts the nonstructural-mass cards (`NSM`, `NSM1`, `NSML`, `NSML1`, `NSMADD`) from a full NX Nastran deck into a standalone BDF, so they can be included once from a master deck. Companion to *export-contact-cards*. Exists because Femap will not write NSM cards for a group-filtered export — confirmed from Femap's own Analysis Set Manager, with no analysis-set switch for it — and because recovering them inside the per-group exporter is worse than the problem: NSM regions do not respect group boundaries, and a region straddling two groups cannot be split between their files without silently changing the mass, since the total-mass forms distribute a total across the elements listed. Cards are copied verbatim rather than generated, so nothing depends on reconstructing which card a `MassType` maps to or how SIDs are assigned. Comments are kept with the card they label.

**Use case:** Your group-by-group export comes out with no nonstructural mass — run this once, include the file once, and it's back.

### Relative Displacement MPC
Instruments the relative displacement between two grids so it comes out of the solve as an output quantity, instead of being recovered by hand-subtracting two result vectors per direction, per load case, per mode. Pick node A (measured *from*) and node B; the tool creates one **measurement node** at their midpoint and writes one MPC constraint equation per requested translation — `1.0*u_M - 1.0*u_A + 1.0*u_B = 0`, so `u_M = u_A - u_B` — after which the measurement node's T1/T2/T3 *are* the relative displacement, plotting and contouring like any other nodal result in statics, modal and frequency response alike. Loops until cancelled, so a dozen joints is a dozen pairs of clicks with one options dialog up front. Because a nodal DOF is expressed in that node's *output* coordinate system, the tool gates every pair on A and B sharing one output CSys that is rectangular, and gives the measurement node that same `outCSys` — cylindrical and spherical are refused rather than handled, since their directions are position-dependent and the subtraction would silently mix directions. A colour-coded axis triad (Femap User Graphics, genuinely erasable) is drawn at both nodes before anything is written, and each pair is recorded in a `RelDisp <A>-<B>` group because Femap nodes have no title and the Messages window is not a record. The measurement node's rotations are deliberately left singular for `PARAM,AUTOSPC`, which the summary warns about on every writing run. A grid node rather than a SPOINT: three SPOINTs would put every reading in some scalar point's T1 column, interpretable only next to a lookup table.

**Use case:** Bolted joint slip, gap closure, or bracket deflection relative to its mount — any "how far did A move with respect to B" question that has to be answered across a lot of load cases or modes.

### Check MPC Export
Diagnostic for the case where constraint equations exist in the model but come out of a Nastran export as nothing at all — no `MPC` cards, no warning, no empty section. Three independent gates can each cause exactly that and are indistinguishable from the symptom: the equations being in a different constraint set than assumed, the analysis set not naming that set in `BCSet[1]` (a separate slot from `BCSet[0]` constraints, and unset on any fresh analysis set), or `NasBulkGroupID` dropping them because a node the equation references is not in the exported group. Rather than reasoning about which, the tool lists every constraint set with the count of equations actually in it, prints the node:dof terms of the equations in the set you pick, then writes two real decks — one whole-model with `BCSet[1]` set, one filtered to a group you choose — and counts the `MPC` cards in each. Comparing the two counts localises the failure to one gate. Nothing in the model is modified; the probe analysis set is deleted again.

**Use case:** An export that should carry constraint equations carries none, and you need to know which gate is eating them before changing anything.
